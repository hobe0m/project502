...
public class CategoryInfoService {
    ...
    public List<Category> getList(boolean isAll) {
        ...
        if(!isAll) { // 사용중인 분류만 조회(ALL이 아니기 때문)
            builder.and(category.active.eq(true));
        }

        List<Category> items = (List<Category>) categoryRepository
                .findAll(builder, Sort.by(desc("listOrder"),
                                          desc("createdAt")));
        return items;
    }
    ...
    public List<Category> getList() {
        ...
        return getList(false); // 사용중 목록 반환(프론트)
    }
}
